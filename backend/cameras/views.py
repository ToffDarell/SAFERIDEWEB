...
class CameraViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        # Operators can read cameras, stream access handles auth manually,
        # YOLO can send heartbeats, and only admins can create, edit, or delete.
    ...
    def stream(self, request, pk=None):
        ...
        if not (user.is_staff or user.is_superuser) and (not profile or profile.status != 'approved'):
            return Response({"error": "Account is not approved"}, status=403)
