...
class SystemSettingsView(APIView):
    ...
    def stream(self, request, pk=None):
        ...
        logger.info(f"Stream opened for: {camera.name}")
        ...
        def generate_frames():
            ...
            frame_count = 0
            # Store last detections so they persist across frames (fixes blinking)
            last_detections = []
            ...
            try:
                ...
                while True:
                    ...
                    # Run YOLO every 3rd frame, but DRAW on every frame
                    if model and frame_count % 3 == 0:
                        try:
                            results = model(frame, conf=0.4, verbose=False)[0]
                            # Update last_detections with new results
                            last_detections = []
                    ...
                    # Always draw last known detections on every frame (no blinking)
